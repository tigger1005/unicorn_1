...
pub fn cached_nop_simulation_x_y(
    ...
    for rec in recs.iter_mut() {
        rec.set_fault_type(FaultType::NopCached(num_x));
    }
    // put every entry in a vec struct
    let mut vec_of_vec_attacks = Vec::new();
    recs.iter()
    ...
    //             run_simulation(file_data, *temp_record, s);
    //         } else {
...
fn run_simulation(
    file_data: &ElfFile,
    records: Vec<Vec<SimulationData>>,
    s: &mut Sender<Vec<Vec<FaultData>>>,
) {
    let mut simulation = Simulation::new(file_data);
